...
function generateURL(
	...
	let video_url = 'https://www.youtube.com/embed/' + video_id + '?rel=0&fs=0';
	if (player_autoplay === '1' || player_autoplay === 1) {
		// Note: Chromium browsers do not allow autoplay in most cases. However, muted autoplay is always allowed.
		video_url += '&autoplay=1&mute=1';
	} else {
...
jQuery('head').append(
	'<style>.cp-modal .cp-youtube .cp-youtube-cta-button .cp-submit { padding: 10px 20px; }</style>'
);

jQuery(document).ready(function () {
	jQuery('html').css('overflow', 'hidden');

	jQuery('body').on('click', '.cp-form-container', function (e) {
		parent.setFocusElement('cta_bg_color');
		e.stopPropagation();
	});
	...
	jQuery('body').on('click', '.cp-affilate-link', function (e) {
		parent.setFocusElement('affiliate_title');
		e.stopPropagation();
	});
	...
	const iframe = jQuery('.cp-content-container').find('iframe'),
		video_id =
			jQuery('#smile_video_id', window.parent.document).val() || '',
		video_start =
			jQuery('.video_start', window.parent.document).val() || '',
		player_actions =
			jQuery('#smile_player_actions', window.parent.document).val() || '',
		player_controls =
			jQuery('#smile_player_controls', window.parent.document).val() ||
			'',
		player_autoplay =
			jQuery('#smile_player_autoplay', window.parent.document).val() ||
			'';
...
jQuery(document).on('smile_data_received', function (e, data) {
	const cp_youtube_submit = jQuery('#youtube_submit_btn');
	let youtube_submit = data.youtube_submit;
...
function update_video_url(data) {
	...
	const old_url = cp_youtube_iframe.attr('data-url');
	if (new_url !== old_url) {
...
function toggle_hide_custom_width() {
	const v = jQuery('#smile_modal_size', window.parent.document).val();
	if (typeof v !== 'undefined') {
		const f_width = jQuery(
			'input[name="cp_modal_width"]',
...
jQuery(document).ready(function () {
	...
	parent
		...
		.on('cp-slider-slide', function (e, el, value) {
...
function update_width(data) {
	...
	const cp_content_container = jQuery('.cp-content-container'),
		cp_modal = jQuery('.cp-modal'),
		cp_modal_content = jQuery('.cp-modal-content');

	let v_height = cp_modal_width;
	v_height *= 1;
	const valueHeight = Math.round((v_height / 16) * 9);
