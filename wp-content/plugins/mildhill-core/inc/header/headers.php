...
class MildhillCoreHeaders {
	...
	public function __construct() {
		...
		add_action( 'mildhill_action_after_page_header_inner', array( $this, 'scroll_appearance' ) );
	...
	function include_elements() {
		...
		foreach ( glob( MILDHILL_CORE_INC_PATH . '/header/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}
		foreach ( glob( MILDHILL_CORE_INC_PATH . '/header/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
		foreach ( glob( MILDHILL_CORE_INC_PATH . '/header/scroll-appearance/*/include.php' ) as $appearance ) {
			include_once $appearance;
		}
		include_once MILDHILL_CORE_INC_PATH . '/header/top-area/include.php';
	}

	function set_variables() {
		$layout_meta = mildhill_core_get_post_value_through_levels( 'qodef_header_layout' );
		$layouts     = apply_filters( 'mildhill_core_filter_register_header_layouts', $header_layouts_option = array() );
	...
	function render_template() {
		...
		if ( ! empty( $header_object ) ) {
			$template_hook = $header_object->overriding_whole_header ? 'mildhill_filter_header_template' : 'mildhill_filter_header_content_template';
	...
	function add_body_classes( $classes ) {
		$header_layout            = mildhill_core_get_post_value_through_levels( 'qodef_header_layout' );
		$header_skin              = mildhill_core_get_post_value_through_levels( 'qodef_header_skin' );
		$header_scroll_appearance = mildhill_core_get_post_value_through_levels( 'qodef_header_scroll_appearance' );
		$header_bottom_shadow     = mildhill_core_get_post_value_through_levels( 'qodef_header_bottom_shadow' );

		$classes[] = ( ! empty( $header_skin ) && 'none' !== $header_skin && ! is_404() ) ? 'qodef-header--' . $header_skin : '';
		...
		$classes[] = ( ! empty( $header_bottom_shadow ) && ( 'yes' === $header_bottom_shadow ) ) ? 'qodef-header--has-bottom-shadow' : '';
	...
	function scroll_appearance() {
		...
		if ( ! empty( $header_object ) ) {
			$appearance_type = mildhill_core_get_post_value_through_levels( 'qodef_header_scroll_appearance' );

			if ( file_exists( MILDHILL_CORE_HEADER_LAYOUTS_PATH . '/' . $header_object->slug . '/templates/' . $appearance_type . '.php' ) ) {
				$scroll_appearance_layout = 'layouts/' . $header_object->slug;
			} else {
				$scroll_appearance_layout = 'scroll-appearance/' . $appearance_type;
			}

			mildhill_core_template_part( 'header/' . $scroll_appearance_layout, 'templates/' . $appearance_type, '', array() );
		}
	}
	...
	function add_header_widget_areas() {
		register_sidebar(
			array(
				...
				'name'          => esc_html__( 'Header - Area One', 'mildhill-core' ),
				'description'   => esc_html__( 'Widgets added here will appear in header widget area one', 'mildhill-core' ),
		...
		register_sidebar(
			array(
				...
				'name'          => esc_html__( 'Header - Area Two', 'mildhill-core' ),
				'description'   => esc_html__( 'Widgets added here will appear in header widget area two', 'mildhill-core' ),
		...
		do_action( 'mildhill_core_action_additional_header_widgets_area' );
	}
}

MildhillCoreHeaders::get_instance();