...
class MildhillCoreTopArea {
	...
	public function __construct() {
		...
		add_action( 'mildhill_action_before_page_header', array( $this, 'load_template' ) );
		...
		add_filter( 'mildhill_filter_add_inline_style', array( $this, 'set_inline_top_area_styles' ) );
	}
	...
	function is_top_area_enabled() {
		$top_area_enabled = mildhill_core_get_post_value_through_levels( 'qodef_top_area_header' ) === 'yes';

		if ( is_404() ) {
			$top_area_enabled = true;
		}
	...
	public function set_top_area_height() {
		if ( $this->is_top_area_enabled ) {
			$height         = mildhill_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
			$top_bar_height = ! empty( $height ) ? $height : 47;
	...
	public function is_top_area_transparent() {
		$background_color = mildhill_core_get_post_value_through_levels( 'qodef_top_area_header_background_color' );
	...
	public function load_template() {
		$parameters = array(
			'show_top_area' => $this->is_top_area_enabled
		);

		mildhill_core_template_part( 'header/top-area/', 'templates/top-area', '', $parameters );
	}
	...
	public function set_inline_top_area_styles( $style ) {
		...
		$background_color = mildhill_core_get_post_value_through_levels( 'qodef_top_area_header_background_color' );
		$height           = mildhill_core_get_post_value_through_levels( 'qodef_top_area_header_height' );
		$padding          = mildhill_core_get_post_value_through_levels( 'qodef_top_area_header_side_padding' );
...
MildhillCoreTopArea::get_instance();