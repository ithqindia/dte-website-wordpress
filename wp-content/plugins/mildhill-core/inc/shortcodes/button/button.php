...
if ( ! function_exists( 'mildhill_core_add_button_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function mildhill_core_add_button_shortcode( $shortcodes ) {
		$shortcodes[] = 'MildhillCoreButtonShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'mildhill_core_filter_register_shortcodes', 'mildhill_core_add_button_shortcode', 9 );
}

if ( class_exists( 'MildhillCoreShortcode' ) ) {
	class MildhillCoreButtonShortcode extends MildhillCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'mildhill_core_filter_button_layouts', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( MILDHILL_CORE_SHORTCODES_URL_PATH . '/button' );
			$this->set_base( 'mildhill_core_button' );
			$this->set_name( esc_html__( 'Button', 'mildhill-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays button with provided parameters', 'mildhill-core' ) );
			...
			$this->set_option( array(
				...
				'options'    => array(
					''      => esc_html__( 'Normal', 'mildhill-core' ),
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'target',
				'title'         => esc_html__( 'Target', 'mildhill-core' ),
			...
			$this->set_option( array(
				'name'       => 'hover_color',
				'field_type' => 'color',
				'title'      => esc_html__( 'Text Hover Color', 'mildhill-core' ),
				'group'      => esc_html__( 'Style', 'mildhill-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'html_type',
				'title'         => esc_html__( 'HTML Type', 'mildhill-core' ),
				'options'       => array(
					'default' => esc_html__( 'Default', 'mildhill-core' ),
					'input'   => esc_html__( 'Input', 'mildhill-core' ),
					'submit'  => esc_html__( 'Submit', 'mildhill-core' )
				),
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'input_name',
				'title'      => esc_html__( 'Input Name', 'mildhill-core' ),
				'visibility'    => array(
					'map_for_page_builder'    => false
				)
			) );
			$this->set_option( array(
				'field_type' => 'array',
				'name'       => 'custom_attrs',
				'title'      => esc_html__( 'Custom Data Attributes', 'mildhill-core' ),
				'visibility'    => array(
					'map_for_page_builder'    => false
				)
			) );
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'mildhill_core_button', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return mildhill_core_get_template_part( 'shortcodes/button', 'variations/'.$atts['button_layout'].'/templates/' . $atts['html_type'], '', $atts );
		}
