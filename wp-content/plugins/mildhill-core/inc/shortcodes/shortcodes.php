...
if ( ! class_exists( 'MildhillCoreShortcodes' ) ) {
	class MildhillCoreShortcodes {
		...
		function init() {
			...
			include_once MILDHILL_CORE_SHORTCODES_PATH . '/dashboard/customizer/shortcodes-customizer-options.php';
		}
		...
		function set_enabled_shortcodes() {
			...
			foreach ( glob( MILDHILL_CORE_SHORTCODES_PATH . '/*', GLOB_ONLYDIR ) as $shortcode ) {
				...
				if ( basename( $shortcode ) !== 'dashboard' ) {
					$is_disabled = mildhill_core_performance_get_option_value( $shortcode, 'mildhill_core_performance_shortcode_' );
		...
		function include_shortcode_classes() {
			include_once MILDHILL_CORE_SHORTCODES_PATH . '/shortcode.php';
			include_once MILDHILL_CORE_SHORTCODES_PATH . '/list-shortcode.php';
		}
	...
	MildhillCoreShortcodes::get_instance();
}