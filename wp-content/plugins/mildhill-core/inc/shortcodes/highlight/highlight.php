...
if ( ! function_exists( 'mildhill_core_add_highlight_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function mildhill_core_add_highlight_shortcode( $shortcodes ) {
		$shortcodes[] = 'MildhillCoreHighlightShortcode';

		return $shortcodes;
	}

	add_filter( 'mildhill_core_filter_register_shortcodes', 'mildhill_core_add_highlight_shortcode' );
}

if ( class_exists( 'MildhillCoreShortcode' ) ) {
	class MildhillCoreHighlightShortcode extends MildhillCoreShortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( MILDHILL_CORE_SHORTCODES_URL_PATH . '/highlight' );
			$this->set_base( 'mildhill_core_highlight' );
			$this->set_name( esc_html__( 'Highlight', 'mildhill-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays highlight with provided parameters', 'mildhill-core' ) );
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Disable Text Line Break', 'mildhill-core' ),
				'description'   => esc_html__( 'Enabling this option will disable text line breaks for screen size 1024 and lower', 'mildhill-core' ),
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Highlight Text Position', 'mildhill-core' ),
				'description' => esc_html__( 'Enter the positions of the words you would like to display as "highlight" text. Separate start and end positions with commas (e.g. if you would like to wrap from fifth to ninth words, you would enter "5,9") or if you want to highlight whole text fill -1', 'mildhill-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'highlight_color',
				'title'      => esc_html__( 'Highlight Text Color', 'mildhill-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'highlight_background_color',
				'title'      => esc_html__( 'Highlight Text Background Color', 'mildhill-core' )
			) );
			$this->set_option( array(
				'field_type'    => 'text',
				'name'          => 'link',
		...
		public function render( $options, $content = null ) {
			...
			return mildhill_core_get_template_part( 'shortcodes/highlight', 'templates/highlight', '', $atts );
		}
