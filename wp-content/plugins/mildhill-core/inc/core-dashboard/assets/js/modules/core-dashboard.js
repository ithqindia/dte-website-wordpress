(function ($) {
    ...
    var qodefImport = {
        ...
        importAttachments: function (repeat) {
            ...
            if (!repeat) {
                for (var i = 1; i < 20; i++) {
                    var xml = i < 20 ? 'mildhill_content_' + i + '.xml' : 'mildhill_content_' + i + '.xml';
                    var data = {
                        action: 'content',
                        xml: xml,
                        images: qodefImport.importImages
        ...
        registrationAjax: function (options) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.status == 'success') {
                        location.reload();
                    } else if (response.status == 'error' && typeof response.data['purchase_code'] !== 'undefined' && response.data['purchase_code'] === false) {
