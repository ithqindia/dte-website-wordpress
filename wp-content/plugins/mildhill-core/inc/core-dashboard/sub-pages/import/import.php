...
class MildhillCoreImport {
	...
	public function set_import_url() {
		$params = MildhillCoreDashboard::get_instance()->get_import_params();
	...
	public function import_action() {
		...
		if ( isset( $_POST ) || ! empty( $_POST ) || isset( $_POST['options']['demo'] ) ) {
			...
			if ( ! MildhillCoreDashboard::get_instance()->check_purchase_code( $_POST['options']['demo'] ) ) {
				qode_framework_get_ajax_status( 'error', esc_html__( 'Please don\'t try to hack me. Purchase code registered is not valid', 'mildhill-core' ) );
				exit;
			}
				...
				switch ( $_POST['options']['action'] ):
					...
					case 'rev-slider':
						if ( qode_framework_is_installed( 'revolution-slider' ) ) {
							...
							$this->set_message( esc_html__( 'Revolution Slider isn\'t installed', 'mildhill-core' ) );
						}
						break;
					case 'content':
	...
	function file_content( $path ) {
		...
		if ( '200' != wp_remote_retrieve_response_code( $response ) ) {
			$this->set_message( $response["response"]['message'] . ' ' . esc_html__( 'Please contact support', 'mildhill-core' ) );
	...
	public function import_widgets( $demo ) {
		...
		if ( $widgets_content ) {
			foreach ( (array) $widgets_content['widgets'] as $mildhill_widget_id => $mildhill_widget_data ) {
				update_option( 'widget_' . $mildhill_widget_id, $mildhill_widget_data );
			}

			$ws = $this->import_sidebars_widgets( $widgets );

			if ( $ws ) {
				$this->set_message( esc_html__( 'Widgets are set for proper sidebar', 'mildhill-core' ) );
				$this->set_data( 'type', 'options' );
				$this->set_status( 'success' );
			}
		}
	...
	public function import_sidebars_widgets( $file ) {
		$mildhill_sidebars = get_option( "sidebars_widgets" );
		unset( $mildhill_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$mildhill_sidebars = array_merge( (array) $mildhill_sidebars, (array) $data['sidebars'] );
			unset( $mildhill_sidebars['wp_inactive_widgets'] );
			$mildhill_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $mildhill_sidebars );
			$mildhill_sidebars['array_version'] = 2;

			wp_set_sidebars_widgets( $mildhill_sidebars );
	...
	public function import_options( $file ) {
		...
		$current_options = get_option( MILDHILL_CORE_OPTIONS_NAME );

		if ( $options ) {
			if ( $current_options != $options ) {
				$result = update_option( MILDHILL_CORE_OPTIONS_NAME, $options );
				if ( $result ) {
					$this->update_options_after_import( $file );
					$this->set_status( 'success' );
					$this->set_data( 'type', 'options' );
					$this->set_message( esc_html__( 'Options imported successfully', 'mildhill-core' ) );
				} else {
					$this->set_status( 'error' );
					$this->set_message( esc_html__( 'Problem occurred during options import', 'mildhill-core' ) );
				}
			} else {
				$this->set_status( 'success' );
				$this->set_data( 'type', 'options' );
				$this->set_message( esc_html__( 'Options are already imported', 'mildhill-core' ) );
			}
		}
	}
	...
	public function import_settings_pages( $file ) {
		...
		$new_ids       = get_transient( '_mildhill_core_imported_posts' );
		$fields_status = true;
		...
		if ( $pages ) {
			...
			if ( ! $fields_status ) {
				$this->set_status( 'error' );
				$this->set_message( esc_html__( 'Problem occurred during settings pages import', 'mildhill-core' ) );
			} else {
				$this->set_status( 'success' );
				$this->set_data( 'type', 'options' );
				$this->set_message( esc_html__( 'Settings pages imported successfully', 'mildhill-core' ) );
			}
		} else {
			$this->set_status( 'error' );
			$this->set_message( esc_html__( 'File doesn\'t exist', 'mildhill-core' ) );
		}
	}
	...
	public function import_menu_settings( $file ) {
		...
		if ( $menus_data ) {
			...
			$this->set_status( 'success' );
			$this->set_data( 'type', 'options' );
			$this->set_message( esc_html__( 'Menus set for proper locations', 'mildhill-core' ) );
		} else {
			$this->set_status( 'error' );
			$this->set_message( esc_html__( 'Problem during menus location set', 'mildhill-core' ) );
		}
	}

	public function import_content( $file, $xml, $attachments, $post_id ) {
		ob_start();
		require_once( MILDHILL_CORE_INC_PATH . '/core-dashboard/sub-pages/import/wordpress-importer.php' );
		...
		$mildhill_import = new WP_Import();
		set_time_limit( 0 );

		$mildhill_import->fetch_attachments = $attachments;
		$returned_value                     = $mildhill_import->import( $file . $xml );

		if ( is_wp_error( $returned_value ) ) {
			$this->set_status( 'error' );
			$this->set_data( 'type', 'content' );
			$this->set_data( 'xml', $xml );
			$this->set_message( esc_html__( 'An error occurred during content import', 'mildhill-core' ) );
		} else {
			$this->set_status( 'success' );
			$this->set_data( 'type', 'content' );
			$this->set_data( 'posts', $this->imported_posts );
			$this->set_message( esc_html__( 'File imported successfully', 'mildhill-core' ) . ' ' . $xml );
		}
	...
	public function rev_sliders() {
		$rev_sliders = array(
			'boxed-slider-1.zip',
			'food-store-slider.zip',
			'fulscreen-showcase-slider.zip',
			'grid-home-slider.zip',
			'landing-1.zip',
			'landing-2.zip',
			'landing-3.zip',
			'main-slider-2-1.zip',
			'organic-food-slider-1.zip',
			'organic-food-slider-2.zip',
			'slider-1.zip',
			'slider-1-1.zip',
		);
		...
		if ( $files_created ) {
			...
			foreach ( $rev_sliders as $rev_slider ) {
				...
				if ( ! $rev_results['success'] ) {
					$this->set_status( 'error' );
					$this->set_message( esc_html__( 'Error while importing rev sliders', 'mildhill-core' ) );
					exit;
				}
			}
			$this->set_status( 'success' );
			$this->set_data( 'type', 'options' );
			$this->set_message( esc_html__( 'Rev sliders imported successfully', 'mildhill-core' ) );
		} else {
			$this->set_status( 'error' );
			$this->set_data( 'type', 'options' );
			$this->set_message( esc_html__( 'Files don\'t exist', 'mildhill-core' ) );
		}
	}
	...
	function update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( MILDHILL_CORE_OPTIONS_NAME );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( MILDHILL_CORE_OPTIONS_NAME, $new_global_values );
		}
	}

	function import_get_demo_urls( $folder ) {
		$demo_urls  = array();
		$domain_url = defined( 'MILDHILL_CORE_PROFILE_SLUG' ) ? str_replace( '/', '', $folder ) . '.' . MILDHILL_CORE_PROFILE_SLUG . '-themes.com/' : '';
	...
	function get_noimage_url( $origin_img_url ) {
		...
		return MILDHILL_CORE_ASSETS_URL_PATH . '/img/' . $noimage_fname;
	}
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && ! empty( $_POST ) && ! empty( $_POST['options']['demo'] ) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'qodef_cd_import_nonce' ) ) {
				...
				$html = mildhill_core_get_template_part( 'core-dashboard/sub-pages/import/templates', 'pages-list', '', array( 'pages' => $pages ) );

				if ( $pages ) {
					qode_framework_get_ajax_status( 'success', '', $html );
				} else {
					qode_framework_get_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'mildhill-core' ), '' );
				}
			}
		}
	...
	public function is_ready_to_import() {
		$info = MildhillCoreSystemInfoPage::get_instance()->get_system_info();
...
MildhillCoreImport::get_instance();
