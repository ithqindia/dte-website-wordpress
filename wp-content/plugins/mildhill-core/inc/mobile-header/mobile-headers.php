...
class MildhillCoreMobileHeaders {
	...
	function include_elements() {
		...
		foreach ( glob( MILDHILL_CORE_INC_PATH . '/mobile-header/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}
		foreach ( glob( MILDHILL_CORE_INC_PATH . '/mobile-header/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
	}
	
	function set_variables() {
		$layout_meta = mildhill_core_get_post_value_through_levels( 'qodef_mobile_header_layout' );
		$layouts     = apply_filters( 'mildhill_core_filter_register_mobile_header_layouts', $header_layouts_option = array() );
	...
	function render_template() {
		...
		if ( ! empty( $header_object ) ) {
			$template_hook = $header_object->overriding_whole_mobile_header ? 'mildhill_filter_mobile_header_template' : 'mildhill_filter_mobile_header_content_template';
	...
	function add_body_classes( $classes ) {
		$header_layout = mildhill_core_get_post_value_through_levels( 'qodef_mobile_header_layout' );
		$classes[] = ! empty( $header_layout ) ? 'qodef-mobile-header--' . $header_layout : '';

		$classes[] = mildhill_core_get_post_value_through_levels( 'qodef_mobile_header_scroll_appearance' ) == 'yes' ? 'qodef-mobile-header-appearance--sticky' : '';
	...
	function add_header_widget_areas() {
		register_sidebar(
			array(
				...
				'name'          => esc_html__( 'Mobile Header', 'mildhill-core' ),
				'before_widget' => '<div id="%1$s" class="widget %2$s qodef-mobile-header-widget-area-one" data-area="mobile-header">',
				'after_widget'  => '</div>',
				'description'   => esc_html__( 'Widgets added here will appear in mobile header widget area', 'mildhill-core' )
			)
		);
	}
}

MildhillCoreMobileHeaders::get_instance();