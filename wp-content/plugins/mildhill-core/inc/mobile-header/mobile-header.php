...
abstract class MildhillCoreMobileHeader {
	...
	public function __construct() {
		...
		add_filter( 'mildhill_filter_add_inline_style', array( $this, 'set_inline_mobile_header_styles' ) );
		add_filter( 'mildhill_core_filter_content_margin_mobile', array( $this, 'get_content_margin' ) );
		add_filter( 'mildhill_core_filter_title_padding_mobile', array( $this, 'get_title_padding' ) );
	...
	public function load_template( $parameters = array() ) {
		$parameters = apply_filters( 'mildhill_core_filter_mobile_header_template', $parameters );
		
		return mildhill_core_get_template_part( 'mobile-header/layouts/' . $this->slug, 'templates/' . $this->slug, '', $parameters );
	}
	
	public function set_inline_mobile_header_styles( $style ) {
		$item_styles = array();
		
		$height           = mildhill_core_get_post_value_through_levels( 'qodef_' . $this->slug . '_mobile_header_height' );
		$background_color = mildhill_core_get_post_value_through_levels( 'qodef_' . $this->slug . '_mobile_header_background_color' );
	...
	function set_header_height() {
		$header_height = mildhill_core_get_post_value_through_levels( 'qodef_' . $this->slug . '_mobile_header_height' );
		$header_height = ! empty( $header_height ) ? intval( $header_height ) : $this->default_header_height;

		$this->header_height = apply_filters( 'mildhill_core_filter_set_mobile_header_height', $header_height );
	}
