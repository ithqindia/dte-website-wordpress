...
if ( ! function_exists( 'mildhill_core_register_breadcrumbs_title_layout' ) ) {
	function mildhill_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'MildhillCoreBreadcrumbsTitle';
		
		return $layouts;
	}
	
	add_filter( 'mildhill_core_filter_register_title_layouts', 'mildhill_core_register_breadcrumbs_title_layout');
}

if ( ! function_exists( 'mildhill_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function mildhill_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'mildhill-core' );
		
		return $layouts;
	}
	
	add_filter( 'mildhill_core_filter_title_layout_options', 'mildhill_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'mildhill_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function mildhill_core_breadcrumbs() {
		
		// Breadcrumbs label
		$labels = apply_filters( 'mildhill_core_filter_breadcrumbs_label', array(
			'home'        => esc_html__( 'Home', 'mildhill-core' ),
			'tag'         => esc_html__( 'Posts tagged "%s"', 'mildhill-core' ),
			'author'      => esc_html__( 'Posted by %s', 'mildhill-core' ),
			'search'      => esc_html__( 'Search results for "%s"', 'mildhill-core' ),
			'404'         => esc_html__( '404 - Page not found', 'mildhill-core' ),
			'query_paged' => esc_html__( '(Page %s)', 'mildhill-core' )
		) );
		
		// Breadcrumbs variables
		$settings = apply_filters( 'mildhill_core_filter_breadcrumbs_settings', array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'mildhill_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}
		
		echo apply_filters( 'mildhill_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}