...
class MildhillCoreTitles {
	...
	public function __construct() {
		...
		add_filter( 'mildhill_filter_page_title_classes', array( $this, 'add_page_title_classes' ) );
		
		// Add title area inline styles
		add_filter( 'mildhill_filter_add_inline_style', array( $this, 'add_inline_styles' ) );
	}
	...
	function include_layouts() {
		...
		foreach ( glob( MILDHILL_CORE_INC_PATH . '/title/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}
		
		foreach ( glob( MILDHILL_CORE_INC_PATH . '/title/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
	}
	
	function set_variables() {
		$layout_meta = mildhill_core_get_post_value_through_levels( 'qodef_title_layout' );
		$layouts     = apply_filters( 'mildhill_core_filter_register_title_layouts', $layouts = array() );
	...
	function render_template() {
		...
		if ( ! empty( $title_object ) ) {
			$template_hook = $title_object->overriding_whole_title ? 'mildhill_filter_title_template' : 'mildhill_filter_title_content_template';
	...
	function add_page_title_classes( $classes ) {
		$layout         = mildhill_core_get_post_value_through_levels( 'qodef_title_layout' );
		$text_alignment = mildhill_core_get_post_value_through_levels( 'qodef_page_title_text_alignment' );
		...
		if ( ! empty( $text_alignment ) ) {
			$classes[] = 'qodef-title-text--' . $text_alignment;
		}
	...
	function add_inline_styles( $style ) {
		...
		$height           	  	  = mildhill_core_get_post_value_through_levels( 'qodef_page_title_height' );
		$title_padding    	  	  = apply_filters( 'mildhill_core_filter_title_padding', 0 );
		$title_padding_mobile 	  = apply_filters( 'mildhill_core_filter_title_padding_mobile', 0 );
		$title_vertical_alignment = mildhill_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$background_color 	  	  = mildhill_core_get_post_value_through_levels( 'qodef_page_title_background_color' );
		$image_params     	  	  = mildhill_core_get_page_title_image_params();
		...
		$title_color = mildhill_core_get_post_value_through_levels( 'qodef_page_title_color' );
...
MildhillCoreTitles::get_instance();