...
if ( ! function_exists( 'mildhill_core_add_simple_line_icons_to_collection' ) ) {
	function mildhill_core_add_simple_line_icons_to_collection( $icons ) {
		$icons[] = 'MildhillCoreSimpleLineIconsPack';
		return $icons;
	}
	
	add_filter( 'qode_framework_filter_add_icon', 'mildhill_core_add_simple_line_icons_to_collection' );
}

if ( class_exists( 'QodeFrameworkIconPack' ) ) {
	class MildhillCoreSimpleLineIconsPack extends QodeFrameworkIconPack {
		...
		public function get_style_url() {
			return MILDHILL_CORE_INC_URL_PATH . '/icons/' . $this->get_base() . '/assets/css/' . $this->get_base() . '.min.css';
		}
