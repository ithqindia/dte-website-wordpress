(function( $, document ) {
	...
	var iconic_woothumbs = {
		...
		init_images: function( product_object ) {
			...
			product_object.images.slick( 'slickSetOption', 'adaptiveHeight', true, false );
			...
			var options = {
				...
				getThumbBoundsFn: function( index ) {
					var pageYScroll = window.pageYOffset || document.documentElement.scrollTop,
						$img = product_object.images.find( '.slick-active img' );
		...
		get_gallery_items: function( product_object ) {
			...
			if ( iconic_woothumbs.els.video_template.length > 0 ) {
				items.push( {
					html: $.trim( iconic_woothumbs.els.video_template.html() )
				} );
			}
...
base.scale = scale;
						...
						//add description
						if(base.options.showDescription&&base.$img.attr('alt')&&$.trim(base.$img.attr('alt'))!='')
						{
