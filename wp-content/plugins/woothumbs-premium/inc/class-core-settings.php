...
class Iconic_WooThumbs_Core_Settings {
	...
	public static function account_getting_started() {
		...
		<div class="iconic-notice iconic-notice--getting-started">
			...
			<p><?php _e( "Thank you for choosing Iconic. We've put together some useful links to help you get started:", 'iconic-woothumbs' ); ?></p>
			...
			<p><strong><?php _e( 'Is something not quite right?', 'iconic-woothumbs' ); ?></strong> <?php printf( __( 'Take a look at our
				<a href="%s" target="_blank">troubleshooting documentation</a>', 'iconic-woothumbs' ), self::get_docs_url( 'troubleshooting' ) ); ?>. <?php _e( 'There is a permanent link to the plugin documentation in the "Support" section below.', 'iconic-woothumbs' ); ?>
			</p>
		</div>
		<?php
	}
	...
	public static function setup_dashboard( $settings ) {
			...
			$settings['sections']['licence'] = array(
				...
				'fields'              => array(
					array(
						...
						'type'     => 'custom',
						'default'  => Iconic_WooThumbs_Core_Licence::admin_account_link(),
					),
					array(
						'id'       => 'account',
		...
		if ( current_user_can( 'manage_options' ) && ! defined( 'ICONIC_DISABLE_DASH' ) ) {
			if ( ! empty( self::$args['cross_sells'] ) ) {
				$cross_sells = Iconic_WooThumbs_Core_Cross_Sells::run( array(
				...
				$settings['sections']['cross-sells'] = array(
					...
					'section_title'       => __( 'Works Well With...', 'iconic-woothumbs' ),
	...
	public static function support_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'https://iconicwp.com/support?utm_source=iconic-woothumbs&utm_medium=insideplugin', __( 'Submit Ticket', 'iconic-woothumbs' ) );
	}
	...
	public static function documentation_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', self::get_docs_url( 'collection' ), __( 'Read Documentation', 'iconic-woothumbs' ) );
	}
	...
	public static function account_link() {
		return sprintf( '<a href="%s" class="button button-secondary" target="_blank">%s</a>', 'https://iconicwp.com/account?utm_source=iconic-woothumbs&utm_medium=insideplugin', __( 'Manage Your Account', 'iconic-woothumbs' ) );
	}
}