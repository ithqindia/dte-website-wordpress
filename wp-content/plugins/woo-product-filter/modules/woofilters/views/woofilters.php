...
class WoofiltersViewWpf extends ViewWpf {
	...
	public function generateOverlayHtml( $settings ) {
		...
		if ( !empty($settings['enable_overlay_word']) && !empty($settings['overlay_word']) ) {
			$html .= esc_html__($settings['overlay_word'], 'woo-product-filter');
		}
	...
	public function generateDescriptionHtml( $filter ) {
		...
		if ($description) {
			$html = '<div class="wfpDescription">' . esc_html__($description, 'woo-product-filter') . '</div>';
		} else {
			$html = '';
		}
		return $html;
	...
	public function generatePriceFilterHtml( $filter, $filterSettings, $blockStyle, $key = 1, $viewId = '' ) {
		...
		$noActive = ( ReqWpf::getVar('min_price', 'all', -1) >= 0 && ReqWpf::getVar('max_price', 'all', -1) >= 0 ) ? '' : 'wpfNotActive';
	...
	public function generateOnSaleFilterHtml( $filter, $filterSettings, $blockStyle, $key = 1, $viewId = '' ) {
		...
		$label = esc_html__($this->getFilterSetting($settings, 'f_checkbox_label', $labels['onsale']), 'woo-product-filter');
	...
	public function generateInStockFilterHtml( $filter, $filterSettings, $blockStyle, $key = 1, $viewId = '' ) {
		...
		foreach ($options as $key) {
			if (isset($optionsAll[$key])) {
				...
				$u->name    = esc_html__($u->name, 'woo-product-filter');
				$u->slug    = $key;
				$inStock[]  = $u;
			}
		}
	...
	public function generateRatingFilterHtml( $filter, $filterSettings, $blockStyle, $key = 1, $viewId = '' ) {
		...
		$filter['settings']['f_frontend_type'] = $type;
		$addText                               = esc_html__($this->getFilterSetting($settings, 'f_add_text', 'and up'), 'woo-product-filter');
		$addText5                              = esc_html__($this->getFilterSetting($settings, 'f_add_text5', '5 only'), 'woo-product-filter');
		...
		foreach ($filterItemList as $filterItem) {
			...
			if ( 'dropdown' === $type || 'mul_dropdown' === $type ) {
				...
				if ( ( empty($filterItem->children) && 0 != $filterItem->parent ) || !$hideParent || 0 != $filterItem->parent ) {
					...
					$html .= '<li data-term-id="' . $filterItem->term_id . '" data-parent="' . $filterItem->parent . '" data-term-slug="' . urldecode($filterItem->slug) . '"' . $addAttrs . '>';
	...
	public function getMultiSelectHtml( $htmlOpt, $settings ) {
		...
		return '<select multiple data-placeholder="' . esc_attr__( $this->getFilterSetting( $settings, 'f_dropdown_first_option_text', 'Select all' ), 'woo-product-filter' ) . '"
