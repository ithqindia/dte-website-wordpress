...
class WoofiltersWpf extends ModuleWpf {
	...
	public function init() {
		...
		add_filter( 'uael_woo_product_query_args', array($this, 'loadShortcodeProductsFilter'), 999, 2); // Ultimate Addons for Elementor -> Woo – Products Widget
	...
	public function forceProductFilter( $query ) {
		...
		if ( isset( $query->query_vars['post_type'] ) && 'product' === $query->query_vars['post_type'] && function_exists( 'debug_backtrace' ) ) {
			...
			if ( is_array( $backtrace ) ) {
				...
				$isElementorPostQuery = ( isset( $backtrace[7]['class'] ) && 'ElementorPro\Modules\QueryControl\Classes\Elementor_Post_Query' === $backtrace[7]['class'] );

				$found = $isBlocks || $isAddons || $isElementorPostQuery;
				if (!$found) {
					$classList = array_column($backtrace, 'class');

					$searchList = array('ElementPack\Modules\Woocommerce\Widgets\Products', 'ElementPack\Modules\Woocommerce\Widgets\WC_Carousel');
		...
		if (!isset($this->renderModes[$id])) {
			...
			if ( !$hideWithoutProducts || 'subcategories' != $displayMode || is_search()) {
				if ( is_product_category() && $displayCategory && $displayMobile ) {
					...
					$mode = 11;
				} else if ( 'all_pages' === $displayOnPage ) {
					if (FrameWpf::_()->isPro()) {
						$mode = 12;
					} else if( is_shop() ) {
