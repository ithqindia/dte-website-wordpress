...
class Team_Member extends Shortcodes {
	...
	function team_member_shortcode( $atts, $content = null ) {
		...
		$atts['content_position'] = str_replace(array('top', 'middle', 'bottom'), array('start', 'center', 'end'), $atts['content_position']);

		// selectors 
        $options['selectors']['item'] = '.team-member-' . $options['box_id'];
        $options['selectors']['ovelay'] = '.team-member' . $options['selectors']['item'] . ' .member-image:before';
        ...
        $options['css'] = array(
            ...
            $options['selectors']['ovelay'] => array(),
		...
		$options['css'][$options['selectors']['ovelay']][] = 'background-color: ' . $atts['overlay_bg'];
        ...
        if ( count( $options['css'][$options['selectors']['ovelay']] ) )
            $options['output_css'][] = $options['selectors']['ovelay'] . '{'.implode(';', $options['css'][$options['selectors']['ovelay']]).'}';
