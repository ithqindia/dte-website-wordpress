...
/* Register SoundCloud shortcode
   -------------------------------------------------------------------------- */
add_shortcode("soundcloud", "et_soundcloud_shortcode");


if( ! function_exists('et_soundcloud_shortcode') ) {
 ...
 */
function et_soundcloud_shortcode($atts, $content = null) {
  ...
  $player_type = et_soundcloud_get_option('player_type', 'visual');
  ...
  $plugin_options = array_filter(array(
    'iframe' => $isIframe,
    'width'  => et_soundcloud_get_option('player_width'),
    'height' => et_soundcloud_url_has_tracklist($shortcode_options['url']) ? et_soundcloud_get_option('player_height_multi') : et_soundcloud_get_option('player_height'),
    'params' => array_filter(array(
      'auto_play'     => et_soundcloud_get_option('auto_play'),
      'show_comments' => et_soundcloud_get_option('show_comments'),
      'color'         => et_soundcloud_get_option('color'),
  ...
  $iframe = et_soundcloud_booleanize($options['iframe']);

  // Remove visual parameter from Flash widget or when it's false because that's the default
  if ($options['params']['visual'] && (!$iframe || !et_soundcloud_booleanize($options['params']['visual']))) {
  ...
  if ($iframe) {
    return et_soundcloud_iframe_widget($options);
  } else {
    return et_soundcloud_flash_widget($options);
  }
 ...
 */
function et_soundcloud_get_option($option, $default = false) {
 ...
 * @return {boolean}
 */
function et_soundcloud_booleanize($value) {
 ...
 * @return {boolean}
 */
function et_soundcloud_url_has_tracklist($url) {
 ...
 */
function et_soundcloud_oembed_params_callback($match) {
 ...
 */
function et_soundcloud_iframe_widget($options) {
  ...
  $height = isset($options['height']) && $options['height'] !== 0
              ? $options['height']
              : (et_soundcloud_url_has_tracklist($options['url']) || (isset($options['params']['visual']) && et_soundcloud_booleanize($options['params']['visual'])) ? '450' : '166');
 ...
 */
function et_soundcloud_flash_widget($options) {
  ...
  $width = isset($options['width']) && $options['width'] !== 0 ? $options['width'] : '100%';
  // Set default height if not defined
  $height = isset($options['height']) && $options['height'] !== 0 ? $options['height'] : (et_soundcloud_url_has_tracklist($options['url']) ? '255' : '81');
...
function et_soundcloud_settings_link($links) {
...
/* Add admin menu */
add_action('admin_menu', 'et_soundcloud_shortcode_options_menu');
function et_soundcloud_shortcode_options_menu() {
  add_options_page('SoundCloud Options', 'SoundCloud', 'manage_options', 'soundcloud-shortcode', 'et_soundcloud_shortcode_options');
...
function et_soundcloud_shortcode_options() {
  if (!current_user_can('manage_options')) {
    wp_die( esc_html__('You do not have sufficient permissions to access this page.', 'xstore-core') );
  }
?>
<div class="wrap">
  ...
  <form method="post" action="options.php">
    ...
    <table class="form-table">
      ...
      <p class="submit">
        <input type="submit" class="button-primary" value="<?php esc_html_e('Save Changes', 'xstore-core') ?>" />
      </p>
