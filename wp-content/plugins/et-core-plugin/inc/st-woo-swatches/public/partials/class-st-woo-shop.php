...
class St_Woo_Shop extends St_Woo_Swatches_Base {
	...
	public function etheme_shop_swatches_position(){
		...
		$swatch_hook = etheme_get_option( 'swatch_position_shop' );
	...
	public function loop_swatch( $size = 'normal' ) {
		...
		$sw_popup = (etheme_get_option( 'swatch_layout_shop' ) == 'popup') ? true : false;
	...
	public function swatch_html( $attribute_type, $taxonomy, $terms, $variations, $selected, $size ) {
		$html = '';
		$layout = etheme_get_option( 'swatch_layout_shop' );
		$custom_class = '';
				...
				if( $terms ) {
					...
					foreach( $terms as $term ) {
						...
						if ( in_array( $term->slug, $variations, true ) ) {
							...
							$color = get_woocommerce_term_meta( $term->term_id, 'st-color-swatch', true );
				...
				if( $terms ) {
					...
					foreach( $terms as $term ) {
						...
						if ( in_array( $term->slug, $variations, true ) ) {
							...
							$image = get_woocommerce_term_meta( $term->term_id, 'st-image-swatch', true );
							$class = ( $selected == $term->slug ) ? 'selected' : '';
							...
							$out .= sprintf(
								...
								esc_attr( $term->slug ),
								esc_attr( $term->name ),
								( $image ) ? wp_get_attachment_image( $image ) : '<img src="'.esc_url( ET_CORE_URL . 'inc/st-woo-swatches/public/images/placeholder.png' ) . '"/>' ,
								esc_attr( $subtype )
							);
						}
					}
				...
				if( $terms ) {
					...
					foreach( $terms as $term ) {
						...
						if ( in_array( $term->slug, $variations, true ) ) {
							...
							$label = get_woocommerce_term_meta( $term->term_id, 'st-label-swatch', true );
